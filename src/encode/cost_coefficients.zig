...
//! decoding-time constants consumed via `fast_cost_model.zig`.
//!
//! Exact numeric values matter — these are the thresholds
//! that decide whether a sub-chunk compresses or stores raw.
...
// Exact numeric values are required because these coefficients drive
// the LZ-vs-entropy cost decisions that pick the final encoding type.
...
/// Computes the `coder.SpeedTradeoff` scalar for the High codec:
