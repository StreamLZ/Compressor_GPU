//! Quick UTF-8 text-detection heuristic.
...
//!
//! Used by `resolveParams` to bump `hasher_k` from 4 to 6 on text inputs
//! at engine levels ≤ 3 — longer minimum matches favor the tANS literal
//! stream on text, which tends to produce shorter literal runs than
//! 4-byte matches.
...
pub fn isBlockProbablyText(block: []const u8) bool {
    ...
    while (i < block.len) {
        ...
        const expected_cont: usize = if (c < min_three_byte_lead) 1
            else if (c < min_four_byte_lead) 2
            else 3;

        const remaining = block.len - i;
        const check_count = @min(expected_cont, remaining);

        for (0..check_count) |j| {
            if (!inRange(block[i + j], cont_lo, cont_hi)) return false;
        }
        i += check_count;
        if (check_count < expected_cont) break;
    }
    return true;
}
