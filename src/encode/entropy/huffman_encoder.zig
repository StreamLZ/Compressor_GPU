...
test "encodeBlock roundtrip via highDecodeBytes" {
    var prng = std.Random.DefaultPrng.init(42);
    const r = prng.random();
    var src: [4096]u8 = undefined;
    for (&src) |*b| b.* = r.int(u8);

    var enc_buf: [8192]u8 = undefined;
    const n = try encodeBlock(&enc_buf, &src);
    ...
    const res = try entropy_dec.highDecodeBytes(
        ...
        true,
        &scratch,
        @as([*]u8, &scratch) + scratch.len,
    );
    try testing.expectEqual(@as(usize, src.len), res.decoded_size);
    try testing.expectEqualSlices(u8, &src, dec_buf[0..res.decoded_size]);
}

test "encodeBlock roundtrip text" {
    const text = "the quick brown fox jumps over the lazy dog. " ** 50;
    var enc_buf: [4096]u8 = undefined;
    const n = try encodeBlock(&enc_buf, text);
    ...
    try testing.expectEqual(@as(usize, text.len), res.decoded_size);
    try testing.expectEqualSlices(u8, text, dec_buf[0..res.decoded_size]);
}
