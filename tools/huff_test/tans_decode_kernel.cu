...
// Aligned readLE32: returns the same value as readLE32(p) for any p, but
// via two ALIGNED 32-bit loads + a funnel shift instead of four byte
// loads. p is generally unaligned (the tANS bit cursor advances 1-3
// bytes per refill). Reads up to 8 bytes from (p & ~3) — the caller's
// buffer must have >= 7 bytes of slack past the last refill position.
__device__ __forceinline__ uint32_t readLE32_aligned(const uint8_t* p) {
    ...
    uint32_t lo = base[0];
    uint32_t hi = base[1];
    return __funnelshift_r(lo, hi, off);           // ({hi:lo} >> off) low 32
}
...
// ══════════════════════════════════════════════════════════════════
//  slzTans32DecodeSharedLutKernel — testbed variant.
//  Identical to slzTans32DecodeKernel, but the per-chunk packed LUT is
//  copied global→shared once at the start and the 32-lane decode loop
//  reads it from shared (~30-cycle) instead of __ldg from global
//  (~200-cycle). NCU showed the global LUT load is 83% Long Scoreboard.
// ══════════════════════════════════════════════════════════════════
extern "C" __global__ void __launch_bounds__(64, 4) slzTans32DecodeSharedLutKernel(
    const uint8_t* __restrict__ src_buf,
    uint8_t*       __restrict__ dst_buf,
    const TansDecChunkDesc* __restrict__ descs,
    uint32_t*      __restrict__ out_status,
    ...
    // ── Copy the per-chunk packed LUT global → shared (coalesced). ──
    __shared__ uint32_t s_lut[2][2048];
    for (int i = lane; i < 2048; i += 32) s_lut[warp_id][i] = packed_lut[i];
    __syncwarp();
    ...
    for (uint32_t i = 0; i < my_sym_count; i++) {
        ...
        uint32_t packed = my_lut[state];        // shared-memory LUT read
        uint32_t bits_x = packed >> 24;
        uint8_t sym = (uint8_t)((packed >> 16) & 0xFF);
        uint32_t w = packed & 0xFFFF;
        uint32_t x = (1u << bits_x) - 1;
...
// ══════════════════════════════════════════════════════════════════
//  slzTans32DecodeSharedLutAlignedKernel — testbed variant.
//  Shared-LUT decode (as above) + the refill uses readLE32_aligned:
//  two aligned 32-bit loads + funnel shift instead of four byte loads.
// ══════════════════════════════════════════════════════════════════
extern "C" __global__ void __launch_bounds__(64, 4) slzTans32DecodeSharedLutAlignedKernel(
    const uint8_t* __restrict__ src_buf,
    uint8_t*       __restrict__ dst_buf,
    const TansDecChunkDesc* __restrict__ descs,
    uint32_t*      __restrict__ out_status,
...
// ══════════════════════════════════════════════════════════════════
//  slzTans32DecodeStagedKernel — testbed variant.
//  Shared-LUT + aligned refill, PLUS the whole chunk's 32 sub-streams
//  are staged into shared memory at kernel start. The refill then reads
//  from shared (~30-cycle) instead of global (~200-cycle) — NCU showed
//  the global refill load was 61% Long Scoreboard after the shared-LUT
//  fix. Costs occupancy (sub-streams + LUT both in shared) but the
//  refill-latency drop is the larger lever.
// ══════════════════════════════════════════════════════════════════
#ifndef TANS_STAGE_BYTES
#define TANS_STAGE_BYTES 12288
#endif
extern "C" __global__ void __launch_bounds__(64, 2) slzTans32DecodeStagedKernel(
    const uint8_t* __restrict__ src_buf,
    uint8_t*       __restrict__ dst_buf,
    const TansDecChunkDesc* __restrict__ descs,
    uint32_t*      __restrict__ out_status,
    ...
    __shared__ uint32_t s_lut[2][2048];
    __shared__ __align__(16) uint8_t s_sub[2][TANS_STAGE_BYTES];
    for (int i = lane; i < 2048; i += 32) s_lut[warp_id][i] = packed_lut[i];
    const uint32_t* my_lut = s_lut[warp_id];
    ...
    uint32_t my_offset = prefix_sum - my_size32;
    uint32_t total_sub = __shfl_sync(0xFFFFFFFFu, prefix_sum, 31);

    if (total_sub > TANS_STAGE_BYTES - 8) {       // wouldn't fit — bail
        __syncwarp();
        if (lane == 0) out_status[chunk_id] = TANS_ERR_BAD_TABLE;
        return;
    }
    // Stage the chunk's 32 sub-streams into shared (coalesced copy).
    for (uint32_t i = lane; i < total_sub; i += 32)
        s_sub[warp_id][i] = sub_data_start[i];
    __syncwarp();

    const uint8_t* my_src = s_sub[warp_id] + my_offset;
