...
const bw_mod = @import("bit_writer.zig");

const ByteHistogram = hist_mod.ByteHistogram;

/// Empirical decode-time cost coefficients for the tANS encoder's
/// rate-distortion budget. Calibrated against the historical CPU codec
/// to match its memcpy-vs-tANS pick on real workloads.
const cost_coeffs = struct {
    const tans_base: f32 = 1015.7;
    const tans_per_src_byte: f32 = 2.93114;
    const tans_per_used_symbol: f32 = 73.1277;
    const tans_per_table_entry: f32 = 1.65052;
};
