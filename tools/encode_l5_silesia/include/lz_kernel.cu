...
#if defined(HARNESS_WARP_CHAIN) && HARNESS_WARP_CHAIN
#include "lz_chain_parser_warp.cuh"
#endif
#if defined(HARNESS_HYBRID_CHAIN) && HARNESS_HYBRID_CHAIN
#include "lz_chain_parser_hybrid.cuh"
#endif
...
#if defined(HARNESS_HYBRID_CHAIN) && HARNESS_HYBRID_CHAIN
#define SCAN_BLOCK_CHAIN scanBlockHybrid
#elif defined(HARNESS_WARP_CHAIN) && HARNESS_WARP_CHAIN
#define SCAN_BLOCK_CHAIN scanBlockChain_warp
#else
#define SCAN_BLOCK_CHAIN scanBlockChain
#endif

        // ── Block 1 pass ────
        {
            uint32_t block1_end = (src_size < LZ_BLOCK_SIZE) ? src_size : LZ_BLOCK_SIZE;
            SCAN_BLOCK_CHAIN(src, src_size,
