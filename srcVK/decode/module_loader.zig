...
// VK adaptation (encode iter-2): NEW SLOT (no CUDA fn-ptr analogue).
// Records a SINGLE vkCmdCopyBuffer with regionCount=N regions on the
// stream's transfer cmdbuf, replacing the encode hot-path's per-chunk
// D2H loop (which sat on procD2HOffset → submit+wait per chunk; 1526
// such pairs on enwik8 ≈ 210 ms under Vulkan-on-WDDM's structural
// per-submit floor of ~137 us). The next procs.stream_sync flushes the
// transfer cmdbuf — ONE submit + ONE wait drains all N copies.
//
// When the destination is page-aligned (procMallocHost / allocHost
// hand back page-aligned pointers) AND large enough to clear the
// D2H_IMPORT_THRESHOLD bound (4 MiB), the VK_EXT_external_memory_host
// fast path imports the host buffer once (LRU-cached via
// tryImportHostBuffer) and vkCmdCopyBuffer DMAs directly from the
// device output into the caller's host pages — zero staging memcpy.
// On every call after the first against the same host_ptr the LRU
// cache hits in tryImportHostBuffer and the per-call cost is just the
// vkCmdCopyBuffer record (microseconds).
//
// Sync mode (stream == 0): rejected — caller must use the existing
// procs.d2h_offset slot. The gather only makes sense when there's a
// stream cmdbuf to batch into and a single end-of-frame stream_sync to
// fire the submit. Returns VK_SUCCESS_RC on n_regions == 0 (nothing to
// do).
fn procD2HOffsetGather(
    ...
    const se = streamEntryFor(stream) orelse return -1; // gather is async-only
    const rc_begin = streamBeginTransferIfNeeded(se);
    if (rc_begin != VK_SUCCESS_RC) return rc_begin;

    // Compute the byte range the gather will touch on the host side so
    // both the import attempt and the staging fallback share one bound.
    // dstOffset is in caller-buffer-local coordinates (matches CUDA's
    // d_output + dst_off pointer-arithmetic shape — caller passes
    // dst_off as both srcOffset (in d_output) AND dstOffset (in the
    // imported host buffer / staging). The encode loop sets both to
    // chunk_descs[i].dst_offset.
    var span_hi: u64 = 0;
    {
        var i: usize = 0;
        while (i < n_regions) : (i += 1) {
            const end = regions[i].dstOffset + regions[i].size;
    ...
    // Fast path: VK_EXT_external_memory_host import on the host dst.
    // tryImportHostBuffer hits the LRU cache on every call after the
    // first against the same (host_ptr, span, usage_src=false). The
    // import path treats the imported VkBuffer as the vkCmdCopyBuffer
    // destination and uses the user-supplied dstOffset values directly.
    //
    // Gate at D2H_IMPORT_THRESHOLD so tiny gathers stay on the cheap
    // staging path (rare in practice — encode's gather span is always
    // MB-scale because per_block_cap * n_blocks dominates).
    const D2H_IMPORT_THRESHOLD: usize = 4 * 1024 * 1024;
    if (span >= D2H_IMPORT_THRESHOLD) {
        if (tryImportHostBuffer(dst_host, span, false)) |imported| {
            markImportInFlight(imported.cache_idx);
            // Translate user regions into VkBufferCopy directly: the
            // layout of vulkan_api.VkBufferCopyRegion matches
            // VkBufferCopy 1:1 (srcOffset, dstOffset, size all u64).
            // imported.region_offset is the leading page-pad bias when
            // dst_host wasn't itself page-aligned; add it to every
            // dstOffset so writes land at the caller's intended bytes.
            const region_ptr = @as([*]const VkBufferCopy, @ptrCast(regions));
            if (imported.region_offset == 0) {
                vkCmdCopyBuffer_fn.?(se.transfer_cmdbuf, src_entry.buffer, imported.buffer, @intCast(n_regions), region_ptr);
            } else {
                // Need to rewrite dstOffsets — stash in a stack-bounded
                // scratch (n_regions worst case ~1526 on enwik8 = 36 KB;
                // page_allocator path is fine for the rare misaligned case).
                const gpa = std.heap.page_allocator;
                ...
                while (k < n_regions) : (k += 1) {
                    scratch[k] = .{
                        .srcOffset = regions[k].srcOffset,
                        .dstOffset = regions[k].dstOffset + imported.region_offset,
                        .size = regions[k].size,
                    };
                }
                vkCmdCopyBuffer_fn.?(se.transfer_cmdbuf, src_entry.buffer, imported.buffer, @intCast(n_regions), scratch.ptr);
            }
            const gpa = std.heap.page_allocator;
            se.pending_imported_d2h.append(gpa, imported) catch {
                clearImportInFlight(imported.cache_idx);
    ...
    // Iter-4 staging fallback: bump-allocate `span` bytes from the
    // stream's staging arena, point all regions at it via dstOffset +
    // bump_base, and queue a single pending_d2h record so the
    // post-fence flush memcpys the gather-output bytes out into
    // dst_host. The single record covers the full [bump_base,
    // bump_base + span) range — pending_d2h_flush memcpys the whole
    // range; unused bytes in the per-region gaps don't matter because
    // the caller only reads bytes at the offsets it explicitly
    // recorded.
    const bump = streamStagingBump(se, span) orelse return -1;
    const gpa = std.heap.page_allocator;
    const scratch = gpa.alloc(VkBufferCopy, n_regions) catch return -1;
    defer gpa.free(scratch);
    var k: usize = 0;
    while (k < n_regions) : (k += 1) {
        scratch[k] = .{
            .srcOffset = regions[k].srcOffset,
            .dstOffset = regions[k].dstOffset + bump,
            .size = regions[k].size,
        };
    }
    vkCmdCopyBuffer_fn.?(se.transfer_cmdbuf, src_entry.buffer, se.staging_buffer, @intCast(n_regions), scratch.ptr);
    se.pending_d2h.append(gpa, .{ .host_dst = dst_host, .staging_off = bump, .size = span }) catch return -1;
    return VK_SUCCESS_RC;
}
